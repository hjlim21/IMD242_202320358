...
void draw() {
  ...
  background(230);
  for (int n = 0; n < 5; n++) {
    house(random(0.1 * width, 0.9 * width), random(height * 0.4, height * 0.75),
      random(100, 200), random(150, 250));
  }
}
...
void house(float x, float y, float houseWidth, float houseHeight) {
  ...
  rect(houseWidth / 4 - windowSize / 2, 2 * houseHeight / 3, windowSize, windowSize);
  rect(3 * houseWidth / 4 - windowSize / 2, 2 * houseHeight / 3, windowSize, windowSize);
  popMatrix();
  popStyle();
}
